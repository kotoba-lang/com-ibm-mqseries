(ns ibm_mqseries.main
  "Kotodama WASM entrypoint for the IBM MQ Series clean-room actor (L4) — Clojure port.
  ...
  facts (`ibm_mqseries.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ibm_mqseries")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"ibm_mqseries.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ibm_mqseries-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ibm_mqseries-compat/ready)
